...
class SVOCopySearchView extends Component {
  ...
  _renderSearchListItem({item, index}){
    return(
      <TouchableOpacity onPress={this._onItemPress.bind(this, item, index)} activeOpacity={0.8} style={{flexDirection:'row', height:ItemH, alignItems:'center'}}>
        <Text style={{color:mainTextGreyColor, fontSize:16, flex:1}}>{item.ccCompanyName}</Text>
      </TouchableOpacity>
    )
  }
  ...
  _search(content){
    if(content){
      ...
      this.props.dispatch( create_service(Contract.POST_GET_SUPERVISE_COPY_COMPANY , {keyword:content}))
        .then( res => {
          if(res){
            this.setState({loading:false, searchData:res.entity.ccCompanyList, searchContent:content})
          }else{
...
const ExportView = connect()(SVOCopySearchView);

module.exports.SVOCopySearchView = ExportView;
