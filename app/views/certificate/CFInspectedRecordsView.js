    ...
    return(
      <TouchableOpacity onPress={this._goDetail.bind(this, item, index)} activeOpacity={0.8} style={{height:ItemH, paddingHorizontal:PaddingHorizontal, backgroundColor:'white'}}>
        <View style={{height:50, flexDirection:'row', alignItems:'center'}}>
          <Text style={{fontSize:16, color:mainTextGreyColor, flex:1}}>{item.date}</Text>
          <Text style={{fontSize:14, color:'red'}}>{item.score}分</Text>
        </View>
        ...
        <View style={{flex:1, flexDirection:'row', alignItems:'center'}}>
          <View style={{flex:1}}>
            <Text style={{fontSize:14, color:mainTextGreyColor}}>持证人：{item.name}<Text>{`\t持证人编号：${item.code}`}</Text></Text>
            <Text style={{fontSize:14, color:mainTextGreyColor, marginTop:10}} numberOfLines={1} >检查内容：{item.content}</Text>
          </View>
