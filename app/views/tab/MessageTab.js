...
class MessageTab extends Component {
  ...
  componentDidMount(){
    ...
    InteractionManager.runAfterInteractions(() => {
      setTimeout(() => { self.setState({loading: false, data:[], data1:[], data2:[]}) }, 500);
      ...
      //       this.setState({loading:false, data:[], data1:[], data2:[]})
      //     }else{
    ...
    return(
      <View style={styles.container}>
        ...
        <ScrollableTabView
          renderTabBar={() => <TabBar />}
          initialPage={1}>
          <SubView tabLabel='通知' data={data}/>
          <SubView tabLabel='活动' data={data1}/>
          <SubView tabLabel='资讯' data={data2}/>
        </ScrollableTabView>
  ...
  renderTitle(){
    return(
      <View style={{height:TitleH, backgroundColor:mainColor, justifyContent:'center', alignItems:'center', paddingHorizontal:PaddingHorizontal, paddingTop:TitleMarginTop}}>
        <Text style={{fontSize:20, color:'white'}}>消息列表</Text>
      </View>
    )
  }
    ...
    return(
      <TouchableOpacity onPress={this._onItemPress.bind(this, item, index)} activeOpacity={0.8} style={{height:ItemH, paddingHorizontal:PaddingHorizontal, backgroundColor:'white'}}>
        ...
        <View style={{flex:1, flexDirection:'row', alignItems:'center'}}>
          ...
          <Image source={null} style={{width:25, height:25, resizeMode:'contain'}} />
        </View>
      </TouchableOpacity>
    )
  }
    ...
    if(data && data.length === 0){
      return(
        <View style={{alignSelf:'center', width:EmptyW, alignItems:'center', marginTop:EmptyMarginTop}}>
          <Text style={{fontSize:18, color:mainTextGreyColor, marginTop:EmptyImageW}}>你还没有消息记录</Text>
        </View>
      )
    }
  }
        ...
        {tabs.map((tab, i) => {
          return (
            <TouchableOpacity key={i} activeOpacity={0.8} onPress={() => this.props.goToPage(i)} style={{flex:1, backgroundColor:'white', alignItems:'center', justifyContent:'center', paddingVertical:12}}>
              <Text style={{fontSize: 16, color: (activeTab === i)? mainColor : mainTextGreyColor}}>{tab}</Text>
...
const ExportView = connect()(MessageTab);

module.exports.MessageTab = ExportView;
